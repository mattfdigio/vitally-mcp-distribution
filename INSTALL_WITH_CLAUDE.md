...
Please help me install the Vitally MCP server. Here's what I need you to do:

1. Detect my Mac architecture (Apple Silicon or Intel)
2. Download the appropriate binary from:
   - Apple Silicon: https://github.com/mattfdigio/vitally-mcp-distribution/releases/latest/download/vitally-mcp-arm64
   - Intel: https://github.com/mattfdigio/vitally-mcp-distribution/releases/latest/download/vitally-mcp-x64
3. Save it to `~/.local/bin/vitally-mcp`
4. Make it executable and remove quarantine: `chmod +x` and `xattr -d com.apple.quarantine`
5. Ask me for my Vitally API credentials:
   - API subdomain (e.g., medscout.rest.vitally.io)
   - API key (starts with sk_live_)
   - Data center (US or EU, default US)
6. Update my Claude Desktop config at `~/Library/Application Support/Claude/claude_desktop_config.json` to add the vitally MCP server with my credentials
7. Tell me to restart Claude Desktop to use the new MCP
